...
interface PendingOrdersResult {
  top_pending_routes: PendingRoute[];
  patterns: {
    total_routes_with_pending: number;
    total_pending_orders: number;
    total_orders: number;
    avg_pending_rate: number;
    critical_routes: number;
    high_severity_routes: number;
    moderate_severity_routes: number;
    low_severity_routes: number;
    top_problem_stores: ProblemStore[];
  };
}
...
function generateReport(result: PendingOrdersResult, topN: number): string {
  let report = '';

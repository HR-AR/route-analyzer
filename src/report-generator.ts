...
function generateDriverStoreReport(data: any, rankCount: number = 10): { report: string; summary: string } {
  ...
  if (problem_routes) {
    ...
    if (problem_routes.high_dwell && problem_routes.high_dwell.length > 0) {
      ...
      report += '-'.repeat(60) + '\n';
      problem_routes.high_dwell.forEach((route: any, idx: number) => {
    ...
    if (problem_routes.high_variance && problem_routes.high_variance.length > 0) {
      report += '\nHIGH VARIANCE (Top 5):\n';
      report += '-'.repeat(60) + '\n';
      problem_routes.high_variance.forEach((route: any, idx: number) => {
        report += `${idx + 1}. ${route['Courier Name']} - ${route.Date}\n`;
...
function generateReturnsReport(data: any): { report: string; summary: string } {
  ...
  if (by_carrier && by_carrier.length > 0) {
    ...
    report += '='.repeat(60) + '\n';
    by_carrier.forEach((carrier: any) => {
      report += `\n${carrier.carrier}:\n`;
  ...
  if (top_drivers && top_drivers.length > 0) {
    ...
    report += '='.repeat(60) + '\n';
    top_drivers.forEach((driver: any, idx: number) => {
...
function generateStoreAnalysisReport(data: any): { report: string; summary: string } {
  ...
  if (carrier_breakdown && carrier_breakdown.length > 0) {
    ...
    report += '='.repeat(60) + '\n';
    carrier_breakdown.forEach((carrier: any) => {
      report += `\n${carrier.carrier}:\n`;
      report += `  Routes: ${carrier.routes} | Orders: ${carrier.total_orders}\n`;

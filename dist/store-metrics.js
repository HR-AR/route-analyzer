...
pythonProcess.on('close', (code) => {
    ...
    results.store_metrics.forEach((store) => {
    ...
    report += '='.repeat(80) + '\n';
    results.best_dph_routes.forEach((route, i) => {
    ...
    report += '='.repeat(80) + '\n';
    results.worst_dph_routes.forEach((route, i) => {
        report += `\n  ${i + 1}. ${route['Courier Name']} (${route.Carrier})\n`;
        report += `     Date: ${route.Date} | Store ${route['Store Id']}\n`;
        report += `     DPH: ${route.DPH.toFixed(2)} del/hr | Delivered: ${route['Delivered Orders']}/${route['Total Orders']}\n`;
        report += `     Planned: ${route['Planned Time Hours'].toFixed(2)} hrs | Actual: ${route['Actual Time Hours'].toFixed(2)} hrs | Variance: ${route['Variance Hours'].toFixed(2)} hrs\n`;
    ...
    report += '='.repeat(80) + '\n';
    results.highest_returns.forEach((route, i) => {
        report += `\n  ${i + 1}. ${route['Courier Name']} (${route.Carrier})\n`;
        report += `     Date: ${route.Date} | Store ${route['Store Id']}\n`;
    ...
    report += '='.repeat(80) + '\n';
    results.highest_pending.forEach((route, i) => {
    ...
    report += '='.repeat(80) + '\n';
    results.top_10_stores.forEach((store, i) => {
    ...
    report += '='.repeat(80) + '\n';
    results.bottom_10_stores.forEach((store, i) => {
        const pendingFlag = store.pending_orders > 0 ? (store.pending_rate > 20 ? '🚨' : '⚠️') : '✓';
    ...
    report += '='.repeat(80) + '\n';
    results.best_returns_stores.forEach((store, i) => {
    ...
    report += '='.repeat(80) + '\n';
    results.best_pending_stores.forEach((store, i) => {
    ...
    report += '='.repeat(80) + '\n';
    results.best_variance_stores.forEach((store, i) => {
    ...
    const belowAvgStores = results.store_metrics.filter((s) => s.avg_dph < overall.avg_dph);
    if (belowAvgStores.length > 0) {
        report += `\n${belowAvgStores.length} stores below average DPH (need improvement):\n`;
        belowAvgStores.slice(0, 5).forEach((store) => {
    ...
    const highVarianceStores = results.store_metrics.filter((s) => s.avg_variance_hours > overall.avg_variance_hours);
    if (highVarianceStores.length > 0) {
        report += `\n${highVarianceStores.length} stores with above-average variance (over planned time):\n`;
        highVarianceStores.slice(0, 5).forEach((store) => {
    ...
    const highDwellStores = results.store_metrics.filter((s) => s.avg_dwell_time > 30);
    const highLoadStores = results.store_metrics.filter((s) => s.avg_load_time > 60);
    if (highDwellStores.length > 0) {
        report += `\n${highDwellStores.length} stores with high avg dwell time (>30 min):\n`;
        highDwellStores.slice(0, 5).forEach((store) => {
    ...
    if (highLoadStores.length > 0) {
        ...
        highLoadStores.slice(0, 5).forEach((store) => {

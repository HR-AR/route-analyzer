...
pythonProcess.on('close', (code) => {
    ...
    results.driver_metrics.forEach((driver) => {
    ...
    results.problem_routes.high_dwell.forEach((route, i) => {
    ...
    results.problem_routes.high_load.forEach((route, i) => {
        report += `\n  ${i + 1}. ${route['Courier Name']} (${route.Carrier}) - ${route.Date}\n`;
        report += `     Load Time:       ${route['Driver Load Time']} min\n`;
        report += `     Dwell Time:      ${route['Driver Dwell Time']} min\n`;
        report += `     DPH:             ${route.DPH.toFixed(2)} del/hr\n`;
    ...
    results.problem_routes.high_variance.forEach((route, i) => {
        report += `\n  ${i + 1}. ${route['Courier Name']} (${route.Carrier}) - ${route.Date}\n`;
    ...
    if (results.problem_routes.pending && results.problem_routes.pending.length > 0) {
        ...
        results.problem_routes.pending.forEach((route, i) => {
    ...
    report += '='.repeat(80) + '\n';
    results.all_routes.forEach((route) => {
...
//# sourceMappingURL=driver-store-analysis.js.map
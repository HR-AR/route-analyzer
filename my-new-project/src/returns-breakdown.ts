...
interface ReturnsBreakdownResult {
  ...
  patterns: {
    ...
    routes_with_both_pending_and_returns: number;
    total_pending_in_return_routes: number;
    avg_pending_when_returns_present: number;
  };
}
...
function generateReport(result: ReturnsBreakdownResult, topN: number): string {
  let report = '';

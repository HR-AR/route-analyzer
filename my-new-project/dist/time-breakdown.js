...
const pythonProcess = spawn(getPythonPath(), ['./scripts/analysis/detailed_time_analysis.py']);
let stdout = '';
let stderr = '';
...
pythonProcess.on('close', (code) => {
    ...
    if (results.both_dwell_and_load_issues && results.both_dwell_and_load_issues.length > 0) {
        ...
        results.both_dwell_and_load_issues.forEach((route, i) => {
    ...
    results.worst_dwell_times.slice(0, 10).forEach((route, i) => {
        report += `\n  ${i + 1}. ${route['Courier Name']} (${route.Carrier})\n`;
        report += `     📅 Date: ${route.Date} | 🏪 Store: ${route['Store Id']}\n`;
    ...
    results.worst_load_times.slice(0, 10).forEach((route, i) => {
        report += `\n  ${i + 1}. ${route['Courier Name']} (${route.Carrier})\n`;
        report += `     📅 Date: ${route.Date} | 🏪 Store: ${route['Store Id']}\n`;
    ...
    const carrierCounts = {};
    results.worst_dwell_times.slice(0, 10).forEach((r) => {
    ...
    results.worst_dwell_times.slice(0, 3).forEach((route) => {
...
//# sourceMappingURL=time-breakdown.js.map
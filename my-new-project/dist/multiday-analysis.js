...
pythonProcess.on('close', (code) => {
    ...
    const multiDayRoutes = results.routes.filter((r) => r.multi_day);
    const singleDayRoutes = results.routes.filter((r) => !r.multi_day);
    if (multiDayRoutes.length > 0) {
        report += '\nMULTI-DAY ROUTES:\n';
        report += '-'.repeat(80) + '\n';
        multiDayRoutes.forEach((route) => {
...
function buildAllStoresReport(results, csvPath) {
    let report = '';
...
//# sourceMappingURL=multiday-analysis.js.map
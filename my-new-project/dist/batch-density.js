...
const pythonProcess = spawn('./venv/bin/python3', ['./scripts/analysis/batch_density_analysis.py']);
let stdout = '';
let stderr = '';
...
pythonProcess.on('close', (code) => {
    ...
    report += '='.repeat(80) + '\n';
    results.store_level.lowest_10_stores.forEach((store, i) => {
    ...
    report += '='.repeat(80) + '\n';
    results.store_level.highest_10_stores.forEach((store, i) => {
        report += `\n${i + 1}. Store #${store.store_id}\n`;
        report += `   Batch Density:    ${store.batch_density} orders/route\n`;
        report += `   Routes:           ${store.route_count}\n`;
        report += `   Total Orders:     ${store.total_orders}\n`;
    ...
    report += '='.repeat(80) + '\n';
    results.route_level.lowest_10_routes.forEach((route, i) => {
    ...
    report += '='.repeat(80) + '\n';
    results.route_level.highest_10_routes.forEach((route, i) => {
        report += `\n${i + 1}. ${route['Courier Name']} (${route.Carrier})\n`;
        report += `   Date:        ${route.Date}\n`;
        report += `   Store:       #${route['Store Id']}\n`;
        report += `   Orders:      ${route['Total Orders']}\n`;
...
//# sourceMappingURL=batch-density.js.map